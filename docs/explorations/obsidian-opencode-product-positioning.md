...
这里的价值是减少隐藏状态。故障可能出现在代码、PATH、XDG、server health、proxy、iframe、theme token、session URL、context part ignore 状态里。
...
Bar 的第一层应保持轻。点开后进入面板或 popover。高风险动作放在明确位置，不能作为一眼看不懂的小图标暴露。移除 context 的语义是远端 part ignore，这件事要在 UI 里显示清楚。
