...
这份文档重新整理 Obsidian、Background 插件、another-opencode-for-obsidian、OpenCode Web UI 之间的背景关系。它不沿用之前的文档，也不依赖之前排查过程里的临时结论。目标是给后续实现一个可审查的计划。
...
3. 给出一个跨两个自维护插件的实现计划：Background 插件拥有 workspace 级背景，another-opencode-for-obsidian 读取这个背景合同，在 iframe 内画同一个 workspace 背景平面的对应切片。

> [!important]
> 这份文档的计划边界是两个插件都由我们自己维护：`/Users/oujinsai/Projects/another-opencode-for-obsidian` 和 `/Users/oujinsai/Projects/obsidian-editor-background`。这个前提改变了解法。OpenCode 插件不再需要在自己的边界里补偿 Background 插件或 CodeMirror 的局部视觉层，Background 插件也可以正式承担 workspace 背景 owner 的职责。
...
这个项目里，Obsidian 是父页面。OpenCode Web UI 是 iframe 里加载的页面。another-opencode-for-obsidian 是把 OpenCode Web UI 放进 Obsidian 的插件。
...
它的价值是让一个系统把主题值暴露出来，另一个系统按名字消费这些值。Obsidian 官方文档列出了大量主题变量，例如 `--background-primary`、`--background-secondary`、`--text-normal`、`--interactive-accent`。another-opencode-for-obsidian 的 Obsidian 外观模式应该优先读取这些稳定变量。
...
1. another-opencode-for-obsidian 的 iframe 内部 point diagnostics。
...
flowchart TB
  ...
  B --> E["another-opencode-for-obsidian ItemView host"]
  E --> F["iframe element"]
...
| another-opencode-for-obsidian host | 父窗口 Obsidian ItemView | iframe 容器、主题采集、消息桥接、诊断 |
...
OpenCode Web UI 不应该在父窗口里直接修改 `.cm-active`、`.markdown-reading-view`、`.cm-editor`。这些 selector 属于 Obsidian editor 和 Background 插件的世界。another-opencode-for-obsidian 可以诊断它们，但不应该靠 patch 它们来隐藏问题。
...
active editor projection 还会让左侧 editor 的局部状态层进入同一个视觉判断：active line、selection、table row、code block、resize handle 都会显得像“背景不连续”。继续在 another-opencode-for-obsidian 里针对这些 selector 打补丁，会把 editor 的局部视觉策略塞进 OpenCode 插件。
...
这个问题的修复入口在 editor 状态层所属的位置：Background 插件或 Obsidian theme CSS。another-opencode-for-obsidian 可以减少额外触发，例如普通 iframe 点击不主动调用 `workspace.setActiveLeaf()`；它不应该覆盖 `.cm-line.cm-active`。
...
1. another-opencode-for-obsidian 找 active Markdown editor 的背景层。
...
2. 或在 another-opencode-for-obsidian host 外面画一层背景。
...
3. another-opencode-for-obsidian 在 Obsidian 外观模式下读取这些变量。
4. another-opencode-for-obsidian 同时知道 iframe 在父窗口 viewport 里的矩形。
...
2. another-opencode-for-obsidian 只消费 owner 发布的变量。
...
这里的“合同”只是两个插件约定一组 CSS 变量和含义。它不需要新服务，不需要数据库，不需要 IPC。Background 插件写变量，another-opencode-for-obsidian 读变量。
...
another-opencode-for-obsidian 应优先读取 `--obsidian-workspace-background-*`。如果这些变量不存在，再退回 A 方案的 Obsidian material。旧 `--obsidian-editor-background-*` 可以作为兼容输入，但不应该继续被建模成 active editor projection 的 source。
...
2. another-opencode-for-obsidian 当前是否处于 `webViewAppearance = "obsidian"`。
...
5. 不再在 another-opencode-for-obsidian 里 patch `.cm-active`、selection、table row。
...
1. another-opencode-for-obsidian 读取 Obsidian stable CSS variables。
...
2. another-opencode-for-obsidian 读取 Background 插件发布的 workspace 变量。
3. another-opencode-for-obsidian 把 iframe rect 和父 viewport 尺寸发给 iframe。
...
3. payload 不出现旧 `--another-opencode-for-obsidian-editor-background-*`。
...
3. another-opencode-for-obsidian 覆盖 `.cm-active`、`.cm-line`、`.markdown-reading-view`、`.cm-editor`。
4. 每个 pane 独立 `background-size: cover`。
5. active editor rect 进入最终背景算法。
6. 旧 `--another-opencode-for-obsidian-editor-background-*` 变量复活。
...
another-opencode-for-obsidian 最终应删除或退役：

1. active editor background image projection。
2. 读取 `.cm-editor::before` 后推导 iframe 背景的生产路径。
3. host pane 背景伪层。
4. 透明 iframe 合成依赖。
5. old `--another-opencode-for-obsidian-editor-background-*` payload。
6. old `--another-opencode-for-obsidian-iframe-*` payload。
...
another-opencode-for-obsidian 应保留：
...
不同 Obsidian community theme 会定义不同颜色。Background 插件和 another-opencode-for-obsidian 应优先使用 Obsidian 官方稳定变量，缺失时才用保守 fallback。
...
OpenCode 上游可能改 token 或组件结构。another-opencode-for-obsidian 应通过 token bridge 处理，不应该 patch OpenCode 组件 class。harness 需要采样大面积元素的 computed style，发现实体背景后回到 token 层处理。
...
3. Background 开启但 contract 缺失或无效时，another-opencode-for-obsidian 退回 A。
4. 不恢复 active editor projection。
5. 不恢复透明 iframe 合成。
6. 不在 another-opencode-for-obsidian 里 patch editor selectors。
...
1. 停止在 another-opencode-for-obsidian 里修 `.cm-active`、resize handle、table row 这类父窗口 editor 层。
...
5. 让 another-opencode-for-obsidian 在 Obsidian 外观模式下消费这个 contract。
6. Background 关闭时，another-opencode-for-obsidian 使用 A。
7. Background 开启时，another-opencode-for-obsidian 使用 workspace 背景切片。
...
这个需要用户肉眼验收。建议先用变量暴露，不在 another-opencode-for-obsidian 写死：
...
Background 插件已经监听 `window-open`，说明它考虑了多个 Obsidian window。workspace contract 也必须按每个 document 发布。another-opencode-for-obsidian 在哪个 Obsidian window 里运行，就读那个 window 的 `document.body`。
...
这一步如果失败，another-opencode-for-obsidian 不应该继续改。背景 owner 自己还没有稳定。

### 第 2 步：another-opencode-for-obsidian 接入 workspace contract

只改 another-opencode-for-obsidian 背景消费路径。
...
最值得继续的方向是：Background 插件画一张 workspace 背景，another-opencode-for-obsidian 在 iframe 内画同一张 workspace 背景的切片。A 作为 Background 关闭或 contract 无效时的回退。B、旧 C、D 不再作为最终方案。
