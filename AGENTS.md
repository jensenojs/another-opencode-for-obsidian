...
- 不要用 OpenCode 内部组件 class selector 重写主题。当前可验收路径是打开 proxy URL，读取内部 DOM 的 computed style，确认 `--background-base`、`--text-strong`、`--border-weak-base` 等 token 已影响页面
- 代理在插件卸载时自动关闭
...
- `webViewAppearance`: `opencode` 保留 OpenCode Web UI 风格；`obsidian` 把 Obsidian 当前主题变量映射到 OpenCode Web UI 内部 token
